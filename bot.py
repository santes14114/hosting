...
KEY_LOG_CHANNEL_ID = 1523965225605402704  # Key log kanalı (aynı log kanalını kullan)
...
# Key veritabanı (geçici - restartta kaybolur, daha kalıcı için JSON dosyası kullanılabilir)
key_database = {}
pending_keys = {}
...
def generate_key(length: int = 16) -> str:
    """Rastgele key oluşturur"""
    characters = string.ascii_uppercase + string.digits
    return ''.join(random.choices(characters, k=length))


def format_key(key: str) -> str:
    """Key'i formatlı şekilde gösterir (XXXX-XXXX-XXXX-XXXX)"""
    if len(key) == 16:
...
def get_key_price(key_type: str) -> int:
    """Key tipine göre fiyat döndürür"""
    prices = {
        "1gun": 150,
        "1hafta": 300,
        "1ay": 500
...
class KeySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Key talep ID'si oluştur
        request_id = f"KEY-{random.randint(10000, 99999)}"
        
        # Geçici key oluştur (onaylanana kadar beklet)
        temp_key = generate_key(16)
        
        # Bekleyen key'leri kaydet
...
# ============================================================
# KEY YÖNETİM KOMUTLARI (Kurucu/Yetkili)
# ============================================================
@bot.command(name="keypanel")
async def keypanel_command(ctx):
    """Key panelini gönderir"""
    embed = discord.Embed(
...
async def keyonay_command(ctx, request_id: str):
    """Key talebini onaylar ve key'i kullanıcıya gönderir"""
...
async def keykontrol_command(ctx, key: str):
    """Key'in geçerliliğini kontrol eder"""
    # Key'i formatla
...
def analyze_token(token: str) -> dict:
    """Bot token'ını detaylı analiz eder"""
    result = {
        "valid_format": False,
    ...
    if "." in token:
        ...
        # Bot token formatı kontrolü (3 parçalı, her parça base64)
        if len(parts) >= 3:
            result["valid_format"] = True
            result["token_type"] = "Discord Bot Token (3 Parçalı)"
            
            # İlk parçadan Bot ID çıkarma
            try:
                first_part = parts[0]
...
def analyze_snowflake(snowflake: int) -> dict:
    """Discord Snowflake ID'yi analiz eder"""
    result = {
        "id": snowflake,
        "is_valid": False,
        "created_at": None,
...
# ============================================================
# DİĞER MODALLAR (Domain, IP, SSL, Port, URL, Ping)
# ============================================================
...
# ============================================================
# TICKET PANELİ OTOMATİK GÖNDERİM
# ============================================================
